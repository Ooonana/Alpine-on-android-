...
V68.1 retains the embedded PRoot-Distro 4.38.0 login path, but its public CLI is restricted to `login`, `list`, and `help`; install/reset/restore/copy and the other mutating management commands are disabled because Alpine-on-Android ships and upgrades its own prepared rootfs.
