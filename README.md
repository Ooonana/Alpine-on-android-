...
V68.4 retains the embedded PRoot-Distro 4.38.0 login path, but its public CLI is restricted to `login`, `list`, and `help`; install/reset/restore/copy and the other mutating management commands are disabled because Alpine-on-Android ships its own prepared rootfs. V68.4 migrates V68.1/V68.2/V68.3 installations in place: only bootstrap-owned launcher/display/banner files and the two version markers are replaced, so packages installed with `apk`, Alpine `/etc` changes outside those owned files, and files under `/root` are preserved. Exiting the Alpine shell now ends the terminal session instead of exposing the hidden Android host shell.
