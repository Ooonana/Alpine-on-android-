package com.alpine.shared.alpine.plugins;
...
import com.alpine.shared.shell.command.result.ResultConfig;
import com.alpine.shared.shell.command.result.ResultData;
...
import com.alpine.shared.shell.command.result.ResultSender;
...
import com.alpine.shared.alpine.settings.preferences.AlpinePreferenceConstants.ALPINE_APP;
...
public class AlpinePluginUtils {

    private static final String LOG_TAG = "AlpinePluginUtils";
        ...
        if (isPluginExecutionCommandWithPendingResult) {
            ...
            if (error != null) {
                ...
                sendPluginCommandErrorNotification(context, logTag, null,
                    ...
                    false, true, AlpineUtils.AppInfoMode.ALPINE_AND_CALLING_PACKAGE,true,
     ...
     *                          {@link ALPINE_APP#KEY_PLUGIN_ERROR_NOTIFICATIONS_ENABLED}
     *                          is {@code false}.
     ...
     *
     * Otherwise if the {@link ALPINE_APP#KEY_PLUGIN_ERROR_NOTIFICATIONS_ENABLED} is
     * enabled, then a flash and a notification will be shown for the error as well
     * on the {@link AlpineConstants#ALPINE_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME} channel instead of just logging
     * the error.
     *
     * @param context The {@link Context} for operations.
     * @param logTag The log tag to use for logging.
    ...
    public static void processPluginExecutionCommandError(final Context context, String logTag,
        ...
        sendPluginCommandErrorNotification(context, logTag, null,
            ResultData.getErrorsListMinimalString(resultData),
            ExecutionCommand.getExecutionCommandMarkdownString(executionCommand),
            forceNotification, true, AlpineUtils.AppInfoMode.ALPINE_AND_CALLING_PACKAGE, true,
            executionCommand.resultConfig.resultPendingIntent != null ? executionCommand.resultConfig.resultPendingIntent.getCreatorPackage(): null);
    }
    ...
    public static void setPluginResultPendingIntentVariables(ExecutionCommand executionCommand) {
        ...
        resultConfig.resultBundleKey = ALPINE_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE;
        resultConfig.resultStdoutKey = ALPINE_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_STDOUT;
        resultConfig.resultStdoutOriginalLengthKey = ALPINE_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_STDOUT_ORIGINAL_LENGTH;
        resultConfig.resultStderrKey = ALPINE_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_STDERR;
        resultConfig.resultStderrOriginalLengthKey = ALPINE_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_STDERR_ORIGINAL_LENGTH;
        resultConfig.resultExitCodeKey = ALPINE_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_EXIT_CODE;
        resultConfig.resultErrCodeKey = ALPINE_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_ERR;
        resultConfig.resultErrmsgKey = ALPINE_SERVICE.EXTRA_PLUGIN_RESULT_BUNDLE_ERRMSG;
    }
    ...
    public static void setPluginResultDirectoryVariables(ExecutionCommand executionCommand) {
        ...
        resultConfig.resultDirectoryPath = AlpineFileUtils.getCanonicalPath(resultConfig.resultDirectoryPath, null, true);
        resultConfig.resultDirectoryAllowedParentPath = AlpineFileUtils.getMatchedAllowedAlpineWorkingDirectoryParentPathForPath(resultConfig.resultDirectoryPath);
    ...
    /**
     * Send a plugin error report notification for {@link AlpineConstants#ALPINE_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID}
     * and {@link AlpineConstants#ALPINE_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     ...
     *                          regardless of if pending intent is {@code null} or
     *                          {@link AlpinePreferenceConstants.ALPINE_APP#KEY_PLUGIN_ERROR_NOTIFICATIONS_ENABLED}
     *                          is {@code false}.
    ...
    public static void sendPluginCommandErrorNotification(final Context currentPackageContext, String logTag,
        ...
        sendPluginCommandErrorNotification(currentPackageContext, logTag,
            ...
            forceNotification, showToast, AlpineUtils.AppInfoMode.ALPINE_AND_PLUGIN_PACKAGE, addDeviceInfo, null);
    }

    /**
     * Send a plugin error notification for {@link AlpineConstants#ALPINE_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID}
     * and {@link AlpineConstants#ALPINE_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     * @param currentPackageContext The {@link Context} of current package.
     * @param logTag The log tag to use for logging.
    ...
    public static void sendPluginCommandErrorNotification(Context currentPackageContext, String logTag,
        ...
        if (alpinePackageContext == null) {
            Logger.logWarn(LOG_TAG, "Ignoring call to sendPluginCommandErrorNotification() since failed to get \"" + AlpineConstants.ALPINE_PACKAGE_NAME + "\" package context from \"" + currentPackageName + "\" context");
            return;
        }
        ...
        if (title == null || title.toString().isEmpty())
            title = AlpineConstants.ALPINE_APP_NAME + " Plugin Execution Command Error";
        ...
        if (appInfoMode != null)
            reportString.append("\n\n").append(AlpineUtils.getAppInfoMarkdownString(currentPackageContext, appInfoMode,
        ...
        setupPluginCommandErrorsNotificationChannel(alpinePackageContext);
        ...
        Notification.Builder builder = getPluginCommandErrorsNotificationBuilder(currentPackageContext, alpinePackageContext,
    ...
    /**
     * Get {@link Notification.Builder} for {@link AlpineConstants#ALPINE_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID}
     * and {@link AlpineConstants#ALPINE_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     * @param currentPackageContext The {@link Context} of current package.
    ...
    public static Notification.Builder getPluginCommandErrorsNotificationBuilder(final Context currentPackageContext,
        ...
        return AlpineNotificationUtils.getAlpineOrPluginAppNotificationBuilder(
            ...
            AlpineConstants.ALPINE_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID, Notification.PRIORITY_HIGH,
            title, notificationText, notificationBigText, contentIntent, deleteIntent, notificationMode);
    }

    /**
     * Setup the notification channel for {@link AlpineConstants#ALPINE_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID} and
     * {@link AlpineConstants#ALPINE_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     * @param context The {@link Context} for operations.
     */
    public static void setupPluginCommandErrorsNotificationChannel(final Context context) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) return;
        NotificationUtils.setupNotificationChannel(context, AlpineConstants.ALPINE_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID,
            AlpineConstants.ALPINE_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME, NotificationManager.IMPORTANCE_HIGH);
    }



    /**
     * Check if {@link AlpineConstants#PROP_ALLOW_EXTERNAL_APPS} property is not set to "true".
     *
    ...
    public static String checkIfAllowExternalAppsPolicyIsViolated(final Context context, String apiName) {
        ...
        AlpineAppSharedProperties mProperties = AlpineAppSharedProperties.getProperties();
        if (mProperties == null || !mProperties.shouldAllowExternalApps()) {
            errmsg = context.getString(R.string.error_allow_external_apps_ungranted, apiName,
                AlpineFileUtils.getUnExpandedAlpinePath(AlpineConstants.ALPINE_PROPERTIES_PRIMARY_FILE_PATH));
        }
