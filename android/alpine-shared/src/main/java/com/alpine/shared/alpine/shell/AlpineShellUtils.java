package com.alpine.shared.alpine.shell;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;

import com.alpine.shared.errors.Error;
import com.alpine.shared.file.filesystem.FileTypes;
...
public class AlpineShellUtils {

    private static final String LOG_TAG = "AlpineShellUtils";
            ...
            try (FileInputStream in = new FileInputStream(file)) {
                ...
                if (bytesRead > 4) {
                    if (buffer[0] == 0x7F && buffer[1] == 'E' && buffer[2] == 'L' && buffer[3] == 'F') {
                            ...
                            if (c == ' ' || c == '\n') {
                                if (builder.length() == 0) {
                                    ...
                                    if (shebangExecutable.startsWith("/usr") || shebangExecutable.startsWith("/bin")) {
                                        ...
                                        interpreter = AlpineConstants.ALPINE_BIN_PREFIX_DIR_PATH + "/" + binary;
                                    }
                                    break;
                                }
                            } else {
                        ...
                        interpreter = AlpineConstants.ALPINE_BIN_PREFIX_DIR_PATH + "/sh";
                    }
                }
            }
        } catch (IOException e) {
    ...
    /** Clear files under {@link AlpineConstants#ALPINE_TMP_PREFIX_DIR_PATH}. */
    public static void clearAlpineTMPDIR(boolean onlyIfExists) {
        ...
        // alpine-reset (d6eb5e35). Moreover, TMPDIR must be a directory and not a symlink, this can
        // also allow users who don't want TMPDIR to be cleared automatically on alpine exit, since
        // it may remove files still being used by background processes (#1159).
        if(onlyIfExists && !FileUtils.directoryFileExists(AlpineConstants.ALPINE_TMP_PREFIX_DIR_PATH, false))
            return;
        ...
        if (days < 0) {
            Logger.logInfo(LOG_TAG, "Not clearing alpine $TMPDIR");
        } else if (days == 0) {
            error = FileUtils.clearDirectory("$TMPDIR",
                FileUtils.getCanonicalPath(AlpineConstants.ALPINE_TMP_PREFIX_DIR_PATH, null));
            if (error != null) {
                Logger.logErrorExtended(LOG_TAG, "Failed to clear alpine $TMPDIR\n" + error);
            }
        } else {
            ...
            if (error != null) {
                Logger.logErrorExtended(LOG_TAG, "Failed to delete files from alpine $TMPDIR older than " + days + " days\n" + error);
            }
        }
    }
