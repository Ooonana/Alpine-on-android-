package com.alpine.shared.alpine.shell.command.runner.terminal;
...
import com.alpine.shared.shell.command.environment.UnixShellEnvironment;
...
import com.alpine.shared.shell.command.environment.IShellEnvironment;
...
/**
 * A class that maintains info for foreground Alpine sessions.
 * It also provides a way to link each {@link TerminalSession} with the {@link ExecutionCommand}
 * that started it.
 */
public class AlpineSession {

    private final TerminalSession mTerminalSession;
    private final ExecutionCommand mExecutionCommand;
    private final AlpineSessionClient mAlpineSessionClient;
    private final boolean mSetStdoutOnExit;

    private static final String LOG_TAG = "AlpineSession";

    private AlpineSession(@NonNull final TerminalSession terminalSession, @NonNull final ExecutionCommand executionCommand,
                          final AlpineSessionClient alpineSessionClient, final boolean setStdoutOnExit) {
        this.mTerminalSession = terminalSession;
        this.mExecutionCommand = executionCommand;
        this.mAlpineSessionClient = alpineSessionClient;
     ...
     * @param alpineSessionClient The {@link AlpineSessionClient} interface implementation.
     ...
     *                        available in the {@link AlpineSessionClient#onAlpineSessionExited(AlpineSession)}
     ...
     * @return Returns the {@link AlpineSession}. This will be {@code null} if failed to start the execution command.
     */
    public static AlpineSession execute(@NonNull final Context currentPackageContext, @NonNull ExecutionCommand executionCommand,
                                        @NonNull final TerminalSessionClient terminalSessionClient, final AlpineSessionClient alpineSessionClient,
        ...
        if (!executionCommand.setState(ExecutionCommand.ExecutionState.EXECUTING)) {
            executionCommand.setStateFailed(Errno.ERRNO_FAILED.getCode(), currentPackageContext.getString(R.string.error_failed_to_execute_alpine_session_command, executionCommand.getCommandIdAndLabelLogString()));
            AlpineSession.processAlpineSessionResult(null, executionCommand);
            return null;
        }

        Logger.logDebugExtended(LOG_TAG, executionCommand.toString());
        Logger.logVerboseExtended(LOG_TAG, "\"" + executionCommand.getCommandIdAndLabelLogString() + "\" AlpineSession Environment:\n" +
            Joiner.on("\n").join(environmentArray));

        Logger.logDebug(LOG_TAG, "Running \"" + executionCommand.getCommandIdAndLabelLogString() + "\" AlpineSession");
        ...
        return new AlpineSession(terminalSession, executionCommand, alpineSessionClient, setStdoutOnExit);
    }

    /**
     * Signal that this {@link AlpineSession} has finished.  This should be called when
     ...
     * and {@link ResultData#exitCode} for the {@link #mExecutionCommand} of the {@code alpineTask}
     * and then calls {@link #processAlpineSessionResult(AlpineSession, ExecutionCommand)} to process the result}.
     *
     */
    public void finish() {
        ...
        if (exitCode == 0)
            Logger.logDebug(LOG_TAG, "The \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" AlpineSession exited normally");
        else
            Logger.logDebug(LOG_TAG, "The \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" AlpineSession exited with code: " + exitCode);

        // If the execution command has already failed, like SIGKILL was sent, then don't continue
        if (mExecutionCommand.isStateFailed()) {
            Logger.logDebug(LOG_TAG, "Ignoring setting \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" AlpineSession state to ExecutionState.EXECUTED and processing results since it has already failed");
            return;
        }
        ...
        AlpineSession.processAlpineSessionResult(this, null);
    }

    /**
     * Kill this {@link AlpineSession} by sending a {@link OsConstants#SIGILL} to its {@link #mTerminalSession}
     * if its still executing.
     *
     * @param context The {@link Context} for operations.
     * @param processResult If set to {@code true}, then the {@link #processAlpineSessionResult(AlpineSession, ExecutionCommand)}
    ...
    public void killIfExecuting(@NonNull final Context context, boolean processResult) {
        ...
        if (mExecutionCommand.hasExecuted()) {
            Logger.logDebug(LOG_TAG, "Ignoring sending SIGKILL to \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" AlpineSession since it has already finished executing");
            return;
        }

        Logger.logDebug(LOG_TAG, "Send SIGKILL to \"" + mExecutionCommand.getCommandIdAndLabelLogString() + "\" AlpineSession");
    ...
    /**
     * Process the results of {@link AlpineSession} or {@link ExecutionCommand}.
     *
     * Only one of {@code alpineSession} and {@code executionCommand} must be set.
     *
     * If the {@code alpineSession} and its {@link #mAlpineSessionClient} are not {@code null},
     * then the {@link AlpineSession.AlpineSessionClient#onAlpineSessionExited(AlpineSession)}
     * callback will be called.
     *
     * @param alpineSession The {@link AlpineSession}, which should be set if
     *                  {@link #execute(Context, ExecutionCommand, TerminalSessionClient, AlpineSessionClient, IShellEnvironment, HashMap, boolean)}
     *                   successfully started the process.
     * @param executionCommand The {@link ExecutionCommand}, which should be set if
     *                          {@link #execute(Context, ExecutionCommand, TerminalSessionClient, AlpineSessionClient, IShellEnvironment, HashMap, boolean)}
     *                          failed to start the process.
     */
    private static void processAlpineSessionResult(final AlpineSession alpineSession, ExecutionCommand executionCommand) {
        if (alpineSession != null)
            executionCommand = alpineSession.mExecutionCommand;

        if (executionCommand == null) return;

        if (executionCommand.shouldNotProcessResults()) {
            Logger.logDebug(LOG_TAG, "Ignoring duplicate call to process \"" + executionCommand.getCommandIdAndLabelLogString() + "\" AlpineSession result");
            return;
        }

        Logger.logDebug(LOG_TAG, "Processing \"" + executionCommand.getCommandIdAndLabelLogString() + "\" AlpineSession result");

        if (alpineSession != null && alpineSession.mAlpineSessionClient != null) {
            alpineSession.mAlpineSessionClient.onAlpineSessionExited(alpineSession);
        } else {
            // If a callback is not set and execution command didn't fail, then we set success state now
            // Otherwise, the callback host can set it himself when its done with the alpineSession
    ...
    public interface AlpineSessionClient {

        /**
         * Callback function for when {@link AlpineSession} exits.
         *
         * @param alpineSession The {@link AlpineSession} that exited.
         */
        void onAlpineSessionExited(AlpineSession alpineSession);
