...
/**
 * Environment for Alpine {@link ExecutionCommand}.
 */
public class AlpineShellCommandShellEnvironment extends ShellCommandShellEnvironment {

    /** Get shell environment containing info for Alpine {@link ExecutionCommand}. */
    @NonNull
    @Override
    public HashMap<String, String> getEnvironment(@NonNull Context currentPackageContext,
        ...
        AlpineAppSharedPreferences preferences = AlpineAppSharedPreferences.build(currentPackageContext);
        ...
        if (ExecutionCommand.Runner.APP_SHELL.equalsRunner(executionCommand.runner)) {
            ...
            ShellEnvironmentUtils.putToEnvIfSet(environment, ENV_SHELL_CMD__APP_SHELL_NUMBER_SINCE_APP_START,
                String.valueOf(AlpineShellManager.getAndIncrementAppShellNumberSinceAppStart()));
            ...
            ShellEnvironmentUtils.putToEnvIfSet(environment, ENV_SHELL_CMD__TERMINAL_SESSION_NUMBER_SINCE_APP_START,
                String.valueOf(AlpineShellManager.getAndIncrementTerminalSessionNumberSinceAppStart()));
        } else {
            return environment;
        }
