...
import com.alpine.shared.alpine.terminal.io.BellHandler;
import com.alpine.shared.logger.Logger;
import com.alpine.terminal.TerminalColors;
import com.alpine.terminal.TerminalSession;
import com.alpine.terminal.TerminalSessionClient;
import com.alpine.terminal.TextStyle;
...
public class AlpineTerminalSessionActivityClient extends AlpineTerminalSessionClientBase {

    private final AlpineActivity mActivity;
    ...
    private static final String LOG_TAG = "AlpineTerminalSessionActivityClient";

    public AlpineTerminalSessionActivityClient(AlpineActivity activity) {
    ...
    public void onStart() {
        ...
        if (mActivity.getAlpineService() != null) {
            setCurrentSession(getCurrentStoredSessionOrLast());
            alpineSessionListNotifyUpdated();
        }
    ...
    public void onSessionFinished(@NonNull TerminalSession finishedSession) {
        AlpineService service = mActivity.getAlpineService();
        ...
        AlpineSession alpineSession = service.getAlpineSession(index);
        if (alpineSession != null) {
            isPluginExecutionCommandWithPendingResult = alpineSession.getExecutionCommand().isPluginExecutionCommandWithPendingResult();
        ...
        if (mActivity.getPackageManager().hasSystemFeature(PackageManager.FEATURE_LEANBACK)) {
            ...
            if (service.getAlpineSessionsSize() > 1 || isPluginExecutionCommandWithPendingResult) {
    ...
    public void onBell(@NonNull TerminalSession session) {
        ...
        switch (mActivity.getProperties().getBellBehaviour()) {
            case AlpinePropertyConstants.IVALUE_BELL_BEHAVIOUR_VIBRATE:
                BellHandler.getInstance(mActivity).doBell();
                break;
            case AlpinePropertyConstants.IVALUE_BELL_BEHAVIOUR_BEEP:
                loadBellSoundPool();
                if (mBellSoundPool != null)
                    mBellSoundPool.play(mBellSoundId, 1.f, 1.f, 1, 0, 1.f);
                break;
            case AlpinePropertyConstants.IVALUE_BELL_BEHAVIOUR_IGNORE:
    ...
    public void setTerminalShellPid(@NonNull TerminalSession terminalSession, int pid) {
        ...
        AlpineSession alpineSession = service.getAlpineSessionForTerminalSession(terminalSession);
        if (alpineSession != null)
    ...
    private synchronized void loadBellSoundPool() {
        if (mBellSoundPool == null) {
            ...
            try {
                mBellSoundId = mBellSoundPool.load(mActivity, com.alpine.shared.R.raw.bell, 1);
            } catch (Exception e){
                // Catch java.lang.RuntimeException: Unable to resume activity {com.alpine/com.alpine.app.AlpineActivity}: android.content.res.Resources$NotFoundException: File res/raw/bell.ogg from drawable resource ID
    ...
    public void switchToSession(boolean forward) {
        ...
        int size = service.getAlpineSessionsSize();
        if (forward) {
        ...
        AlpineSession alpineSession = service.getAlpineSession(index);
        if (alpineSession != null)
            setCurrentSession(alpineSession.getTerminalSession());
    }
    ...
    private void renameSession(TerminalSession sessionToRename, String text) {
        ...
        AlpineService service = mActivity.getAlpineService();
        if (service != null) {
            AlpineSession alpineSession = service.getAlpineSessionForTerminalSession(sessionToRename);
            if (alpineSession != null)
                alpineSession.getExecutionCommand().shellName = text;
        }
    }
    ...
    public void addNewSession(boolean isFailSafe, String sessionName) {
        ...
        if (service.getAlpineSessionsSize() >= MAX_SESSIONS) {
            ...
            AlpineSession newAlpineSession = service.createAlpineSession(null, null, null, workingDirectory, isFailSafe, sessionName);
            if (newAlpineSession == null) return;

            TerminalSession newTerminalSession = newAlpineSession.getTerminalSession();
    ...
    public TerminalSession getCurrentStoredSessionOrLast() {
        ...
        if (stored != null) {
            ...
            AlpineSession alpineSession = service.getLastAlpineSession();
            if (alpineSession != null)
                return alpineSession.getTerminalSession();
            else
                return null;
        }
    }
    ...
    public void removeFinishedSession(TerminalSession finishedSession) {
        ...
        int index = service.removeAlpineSession(finishedSession);
    ...
    public void alpineSessionListNotifyUpdated() {
        mActivity.alpineSessionListNotifyUpdated();
    }
    ...
    public void checkAndScrollToSession(TerminalSession session) {
        ...
        final ListView alpineSessionsListView = mActivity.findViewById(R.id.terminal_sessions_list);
        if (alpineSessionsListView == null) return;

        alpineSessionsListView.setItemChecked(indexOfSession, true);
        // Delay is necessary otherwise sometimes scroll to newly added session does not happen
        alpineSessionsListView.postDelayed(() -> alpineSessionsListView.smoothScrollToPosition(indexOfSession), 1000);
    }
    ...
    public void checkForFontAndColors() {
        try {
            File colorsFile = AlpineConstants.ALPINE_COLOR_PROPERTIES_FILE;
            File fontFile = AlpineConstants.ALPINE_FONT_FILE;
