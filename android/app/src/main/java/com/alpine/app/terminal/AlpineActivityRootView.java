...
/**
 * The {@link AlpineActivity} relies on {@link android.view.WindowManager.LayoutParams#SOFT_INPUT_ADJUST_RESIZE)}
 * set by {@link AlpineTerminalViewClient#setSoftKeyboardState(boolean, boolean)} to automatically
 * resize the view and push the terminal up when soft keyboard is opened. However, this does not
 * always work properly. When `enforce-char-based-input=true` is set in `alpine.properties`
 * and {@link com.alpine.view.TerminalView#onCreateInputConnection(EditorInfo)} sets the inputType
 * to `InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD | InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS`
 * instead of the default `InputType.TYPE_NULL` for alpine, some keyboards may still show suggestions.
 ...
 * like number row, etc.
 *
 * To fix these issues, `activity_alpine.xml` has the constant 1sp transparent
 * `activity_alpine_bottom_space_view` View at the bottom. This will appear as a line matching the
 * activity theme. When {@link AlpineActivity} {@link ViewTreeObserver.OnGlobalLayoutListener} is
 ...
 */
public class AlpineActivityRootView extends LinearLayout implements ViewTreeObserver.OnGlobalLayoutListener {

    public AlpineActivity mActivity;
    ...
    private static final String LOG_TAG = "AlpineActivityRootView";

    private static int mStatusBarHeight;

    public AlpineActivityRootView(Context context) {
        super(context);
    }

    public AlpineActivityRootView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public AlpineActivityRootView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public void setActivity(AlpineActivity activity) {
        mActivity = activity;
    }
    ...
    public void onGlobalLayout() {
        ...
        View bottomSpaceView = mActivity.getAlpineActivityBottomSpaceView();
        ...
        if (isVisible) {
            ...
            // Calling addAlpineActivityRootViewGlobalLayoutListener with a delay fails to
