...
V65 remains the frozen known artifact baseline. V69.3 is the active development line. It uses the reproducibly prepared Alpine 3.23.6 rootfs, Termux PRoot 5.1.107.92, stock Alpine `/sbin/apk`, and the integrated display. V69/V69.1/V69.2/V69.3 add a compact black/white/green input dock below the X11 surface with IME toggle, right click, Esc, Tab, sticky Ctrl/Alt/Meta, arrows, clipboard paste, and hide/show. `install-desktop` now offers XFCE, LXQt, Openbox, MATE, Plasma Desktop, LXDE compatibility, Plasma Mobile, and Phosh; Plasma and Phosh use experimental nested Wayland sessions on the X11 display. The embedded PRoot-Distro 4.38.0 CLI remains restricted to `login`, `list`, and `help`. V69.1 fixed nested-Xwayland socket permissions and added Plasma support packages (Breeze decoration/cursors, KDE portal backend, and PulseAudio X11 utilities). V69.2 forced the Phosh nested compositor to use the pixman renderer, avoiding unavailable DRI3/EGL hardware paths. V69.3 adds Phosh PulseAudio utilities, session-only autostart masks for Android-inapplicable GNOME helpers, and user-controlled Android process-protection diagnostics; it migrates V68.1 through V69.2 in place while preserving installed packages and user data. Android runtime validation is still required for package transactions, DBus, and the embedded display.
