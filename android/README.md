...
V65 remains the frozen known artifact baseline. V68/V68.4 is the active development line. It uses the reproducibly prepared Alpine 3.23.6 rootfs, Termux PRoot 5.1.107.92, stock Alpine `/sbin/apk`, and the integrated display. The embedded PRoot-Distro 4.38.0 CLI is restricted to `login`, `list`, and `help`. V68.4 migrates V68.1/V68.2/V68.3 in place, preserving installed packages and user data while refreshing bootstrap-owned launcher/display/banner files. Exiting Alpine terminates the terminal session rather than dropping into the hidden host shell. Android runtime validation is still required for package transactions, DBus, and the embedded display.
