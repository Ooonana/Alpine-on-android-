...
public class CmdEntryPoint extends ICmdEntryInterface.Stub {
    ...
    static {
        ...
        try {
            if (ctx != null) {
                android.content.pm.ApplicationInfo appInfo = ctx.getPackageManager().getApplicationInfo(BuildConfig.APPLICATION_ID, 0);
                if (appInfo != null && appInfo.nativeLibraryDir != null) {
                    File extractedLib = new File(appInfo.nativeLibraryDir, "libXlorie.so");
                    if (extractedLib.canRead())
                        libPath = extractedLib.getAbsolutePath();
                }
            }
        } catch (Exception e) {
            Log.w("CmdEntryPoint", "Could not resolve extracted native library path", e);
        }
